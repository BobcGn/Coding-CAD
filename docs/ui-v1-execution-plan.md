...
**Non-goals / 非目标**：不接入真实 LLM Provider，生成阶段不调用任何 LLM（P3-D2：LLM 在生成部分无职责，仅限后续审批等支持功能）；不做 Ghost presentation；不实现评论、多人 approval 或 impact analysis。 / No real LLM Provider, and no generation stage invokes any LLM (P3-D2: LLM has no role in generation and is limited to later approval/review support); no Ghost presentation; no comments, multi-approval, or impact analysis.
...
- **P3-D1 Workspace host boundary / Workspace 宿主边界**：已由用户于 2026-08-15 确认——SvelteKit server boundary 调用 Node-only Workspace，浏览器只使用 typed app contract；阻塞 P3.1。 / Confirmed by the user on 2026-08-15 — a SvelteKit server boundary wraps the Node-only Workspace with a typed browser-facing app contract; blocks P3.1.
- **P3-D2 Greenfield generation mode / Greenfield 生成模式**：已由用户于 2026-08-15 确认——LLM 在生成部分无职责，仅负责后续审批等支持功能；requirement -> candidate 完全由 deterministic Architecture Agent + Mock Provider 生成，生成阶段不调用 LLM。真实 LLM 若接入只允许在审批/评审等下游辅助环节（TODO-009 门禁）。阻塞 P3.2。 / Confirmed by the user on 2026-08-15 — LLM has no role in generation and is limited to later approval/review support; requirement -> candidate is fully generated by the deterministic Architecture Agent with the Mock Provider, and no generation stage invokes an LLM. If integrated, a real LLM is allowed only in downstream approval/review support (TODO-009 gate). Blocks P3.2.
- **P3-D3 Inspector command granularity / Inspector 命令粒度**：已由用户于 2026-08-15 确认——app-private、field-specific commands，禁止 generic patch；阻塞 P3.5。 / Confirmed by the user on 2026-08-15 — app-private, field-specific commands with generic patches prohibited; blocks P3.5.
