...
- `.github/workflows/ci.yml`: PR/push gate for linting, changelog traceability, commit format, secret scan, tests, build, dependency review, and dependency audit.
- `.github/workflows/codeql.yml`: CodeQL static security analysis for JavaScript and TypeScript.

- `.github/workflows/ci.yml`：PR/push 门禁，覆盖代码规范、变更记录追溯、提交格式、敏感信息扫描、测试、构建、依赖审查和依赖审计。
- `.github/workflows/codeql.yml`：面向 JavaScript 和 TypeScript 的 CodeQL 静态安全分析。
...
| CodeQL & Security Scanner | `pnpm security:secrets`, `pnpm security:audit`, CodeQL workflow | Covers hardcoded secrets, dependency audit, dependency review, and CodeQL. |
...
| CodeQL & Security Scanner | `pnpm security:secrets`、`pnpm security:audit`、CodeQL workflow | 覆盖硬编码敏感信息、依赖审计、依赖审查和 CodeQL。 |
