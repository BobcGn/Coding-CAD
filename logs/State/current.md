...
Coding CAD 已完成六个核心能力模块：Architecture IR、Component Registry、Architecture Validator、Architecture DSL、CLI 和 Architecture Agent。旧原型目录 `packages/dsl`、`packages/validator` 及其模块日志已按当前主线目录迁移删除。

Coding CAD has completed six core capability modules: Architecture IR, Component Registry, Architecture Validator, Architecture DSL, CLI, and Architecture Agent. The old prototype directories `packages/dsl`, `packages/validator`, and their module logs have been migrated away and removed according to the current mainline directories.

## 已确认事实 / Confirmed Facts

- 项目根目录是 Git 工作树，核心模块迁移、CI/CD 和 CI 修复已分批提交；当前正在实现 Architecture Agent 与 CLI `design` 入口。
- 文档规范要求项目 Markdown 始终中英双语。
- 当前核心闭环是 `requirement` -> `architecture-agent` -> `architecture-ir` -> `architecture-validator` -> refinement，以及 `architecture-dsl` <-> `architecture-ir` -> `cli`。
- 当前已建立推送前本地门禁 `pnpm ci:verify` 和 GitHub Actions CI workflow。
- CodeQL 不再作为自动 workflow 启用，因为远端仓库尚未启用 GitHub code scanning。
- 当前 workspace 模块包括 `apps/web`、`apps/server`、`packages/architecture-ir`、`packages/architecture-dsl`、`packages/component-registry`、`packages/architecture-validator`、`packages/cli`、`packages/architecture-agent`、`packages/agent-runtime`。
- `packages/dsl` 和 `packages/validator` 是旧原型目录，已删除。
- `packages/` 与 `logs/modules/` 中不存在同名重复模块或空目录。
- 六个核心模块已有明确的测试入口；`architecture-agent` 当前使用单元测试覆盖设计、决策、Validator 集成和 refinement loop。

- The project root is a Git worktree; the core module migration, CI/CD, and CI fix have been committed in batches, and the current work implements Architecture Agent and the CLI `design` entry point.
- The documentation rule requires project Markdown to remain bilingual.
- The current core loop is `requirement` -> `architecture-agent` -> `architecture-ir` -> `architecture-validator` -> refinement, plus `architecture-dsl` <-> `architecture-ir` -> `cli`.
...
- Current workspace modules include `apps/web`, `apps/server`, `packages/architecture-ir`, `packages/architecture-dsl`, `packages/component-registry`, `packages/architecture-validator`, `packages/cli`, `packages/architecture-agent`, and `packages/agent-runtime`.
...
- The six core modules have explicit test entry points; `architecture-agent` currently uses unit tests for design, decisions, Validator integration, and the refinement loop.
